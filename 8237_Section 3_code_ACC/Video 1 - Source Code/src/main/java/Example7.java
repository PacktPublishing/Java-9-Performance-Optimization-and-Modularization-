...
public class Example7 {
    ...
    static class BalanceMonitor implements Runnable {
        ...
        public void run() {
            while(true) {
                if (account.tryLock()) { //Balance monitor only needs to read
    ...
    static class Account extends ReentrantLock {
        ...
        public void transferMoney(double amount, Account destinationAccount) {
            ...
            while(!transferred) {
                if(this.tryLock()) { //Needs to change the values
                    if(destinationAccount.tryLock()) { //Needs to change the values
                        this.balance -= amount;
